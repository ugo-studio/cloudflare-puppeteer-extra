...
const debug = Debug("cloudflare-puppeteer-extra");

/**
 * Original Puppeteer API
 * @private
 */
export interface VanillaPuppeteer extends Pick<Puppeteer, "connect"> {}
...
export class PuppeteerExtra implements VanillaPuppeteer {
  ...
  /**
   * The method launches a browser instance with given endpoint.
   *
   ...
   * const browser = await puppeteer.launch(env.MYBROWSER)
   *
   * @param endpoint browser binding: See [puppeteer docs](https://developers.cloudflare.com/browser-rendering/get-started/screenshots/#4-configure-wranglertoml).
   */
  async launch(
 ...
 * @example
 * // js import
 * const { addExtra } = require('cloudflare-puppeteer-extra')
 * const puppeteer = require('@cloudflare/puppeteer')
 *
 * // ts/es6 import
 * import { addExtra } from 'cloudflare-puppeteer-extra';
 * import puppeteer from '@cloudflare/puppeteer'
 *
 * // Patch  @cloudflare/puppeteer and add plugins
...
function requireVanillaPuppeteer(): [VanillaPuppeteer?, Error?] {
  try {
    return [require("@cloudflare/puppeteer"), undefined];
  } catch (err) {
 ...
 * import puppeteer from 'cloudflare-puppeteer-extra'
 *
 * // Add plugins
 * puppeteer.use(...)
 */
